...
namespace Topicos.Netcore.Api.AdventureWorks.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        [HttpGet("PageQuery/")]
        public async Task<ActionResult<IEnumerable<Customer>>> GetCustomersPged(int pageNumber, int pageSize)
        {
            var customer = await _context.Customers.OrderBy(c => c.LastName).Skip(pageSize * (pageNumber - 1)).Take(pageSize).ToListAsync();
            if (customer == null)
            {
                return NotFound();
            }
